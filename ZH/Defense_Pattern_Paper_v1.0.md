...
> **術語說明（Terminology note）。** 上述 `<file>.md` 等占位符（placeholder）對應到本工作所開發的某特定廠商編碼代理 CLI 中的具體檔名。我們採用占位符寫法的理由是：此**架構模式**——system-context 檔 + 永遠載入的 memory index + 延遲載入 entries + skill 設定檔——可推廣到任何暴露對等表面（equivalent surface）的執行環境（見 §5.4 與範本的 "Adapting to your runtime" 章節，內含 OpenAI Assistants API、LangChain、自製代理的映射）。
>
...
攻擊者可以對上述任何檔案寫入任意內容。攻擊者**不能**修改模型的 system prompt 或執行環境的 binary。
...
5/5 的結果只是試點。Wilson 95% CI 下界為 56.6%。我們不宣稱 production-ready，只宣稱：**永遠載入位置的可靠度顯著高於需求載入位置**；試點結果與此假設一致，但尚未建立精確的可靠度數值。
...
本文中的威脅模型、攻擊型錄（已抽象化）、防禦模式，足以在任何暴露 §1 所述使用者可控設定層的 LLM 編碼代理上，建構出可比較的測試平台（test harness）。

我們刻意不公開測試中所用的具體攻擊內容：公開可重用的攻擊 payload，會把攻擊的邊際成本壓低的幅度大於它對防禦研究的助益。

若需以同一批攻擊內容驗證，請私下聯繫作者。
